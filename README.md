...
[Django](https://www.djangoproject.com/) is a high-level Python web framework that encourages rapid development and clean, pragmatic design. It provides a robust set of features for web applications, making it one of the most popular choices for building server-side applications with Python.

To secure such an application, you need a reliable way to handle user logins. For the Django ecosystem, [Authlib](https://authlib.org/) is the standard and recommended library for authentication. Think of it as a flexible security guard for your app. This guide demonstrates how to use Authlib with a Django application to implement a secure login with ZITADEL.
...
This example uses **Authlib**, the standard for Django authentication. While ZITADEL doesn't offer a specific SDK, Authlib is highly modular. It works with Django's integration layer that handles the communication with ZITADEL. Under the hood, this example uses the powerful OIDC standard to manage the secure PKCE flow.

Check out our Example Application to see it in action.

## Example Application

The example repository includes a complete Django application, ready to run, that demonstrates how to integrate ZITADEL for user authentication.
...
>
> These URLs must exactly match what your Django application uses. For production, add your production URLs.
...
### 1. Security headers (Django middleware)

**Not enabled.** Consider adding security headers middleware in your Django application:
```python
MIDDLEWARE = [
